...
def notification():
    if request.method == 'POST':
        ...
        try:
            ...
            servicebus_client = ServiceBusClient.from_connection_string(conn_str, logging_enable=True)
            queue_name = app.config.get('SERVICE_BUS_QUEUE_NAME')
            with servicebus_client: 
                sender_client = servicebus_client.get_queue_sender(queue_name)
                with sender_client: 
                    sendAMessage(notification.id, sender_client)
...
def sendAMessage(notificationId: int, sender: any):
    msg = ServiceBusMessage(str(notificationId))
